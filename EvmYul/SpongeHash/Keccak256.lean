/-
Pure, total Keccak-256 (restored).

This is the equational implementation this file's stub always promised: a
plain Lean sponge over `Nat` lanes, no `partial`, no FFI. The compiled/native
path may still use the `keccak256` C symbol via `EvmYul.ffi` — whose `def`
body is THIS function, so the logical meaning is pure and provable while
execution stays fast.

Provenance: verbatim port of the implementation maintained (corpus-validated
against solc/Forge fixtures and byte-parity-checked against the native
`keccak256` symbol) in the solidity-lean project, itself a relocation of the
former upstream implementation of this module.
-/
...
/-- ByteArray convenience wrapper: hash the whole array. -/
def keccak256 (input : ByteArray) : ByteArray :=
  ByteArray.mk (Array.mk ((keccak256Bytes (input.toList.map UInt8.toNat)).map UInt8.ofNat))
